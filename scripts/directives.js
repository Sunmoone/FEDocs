...
directives.service("myTemplates",function(){
  return {
    ...
    accordiongroup: '<div class="accordion" ng-transclude></div>',
    accordion:   ['<div class="accordion-group">',
                    '<div class="accordion-heading">',
                      '<a class="accordion-toggle" href="" ng-click="toggle=!toggle">{{title}}</a>', 
                    '</div>',
                    '<div  class="accordion-body collapse in">',
                      '<div class="accordion-inner" ng-show="toggle" ng-transclude></div>',
                    '</div>',
                  '</div>'].join(""),
...
directives.directive('accordiongroup', function(myTemplates){
  return {
    restrict: 'E',
    template: myTemplates.accordiongroup,
    transclude: true,
    replace: true
  }
});

directives.directive('accordion', function(myTemplates){
  return {
    restrict: 'E',
    require: '^accordiongroup',
    scope:{title: '@'},
    template: myTemplates.accordion,
    transclude: true,
    replace: true
  }
});
...
directives.directive('resizy',function(){
  return {
    restrict: 'A',
    link: function(scope,element,attrs,ctrl){
      var resiz1 = new Rsz();
      resiz1.obj = element[0];
      //resiz1.preventY = true;
      resiz1.init();  
    }
  }
});
