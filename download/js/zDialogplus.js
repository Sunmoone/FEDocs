...
if(document.compatMode=="BackCompat") ielt7 = true;
 ...
 var $topWindow = function () {
         ...
         try{
           ...
           while (parentWin != parentWin.parent) {
              if (parentWin.parent.document.getElementsByTagName("iframe").length) break;
...
Dialog.prototype.create = function () {
    ...
    var DialogDivWidth = this.Width + 5 + 5;
    var DialogDivHeight = this.Height + 33 + 5 + (this.ShowButtonRow ? 40 : 0) + (this.ShowMessageRow ? 50 : 0);
  ...
  <table id="_DialogTable_{thisID}" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: -moz-none; font-size:12px; line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <td height="33" style="background-image:url({IMAGESPATH}dialog_ct.png) !important;background: url({IMAGESPATH}dialog_ct.gif) repeat-x top;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><img align="absmiddle" src="{IMAGESPATH}icon_dialog.gif"/><span id="_Title_{thisID}">' + this.Title + '</span></div>\
        <div onclick="Dialog.zoom({thisID})" id="_ButtonZoom_{thisID}" style="display:none;background:url({IMAGESPATH}/dialog_zoombtn.gif);height:17px;width:28px;position:absolute;right:50px;top:8px;cursor:pointer;background-repeat:no-repeat;"></div><div id="_ButtonClose_{thisID}" onclick="Dialog.getInstance(\'{thisID}\').cancelButton.onclick.apply(Dialog.getInstance(\'{thisID}\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url({IMAGESPATH}dialog_closebtn.gif)\'" onmouseover="this.style.backgroundImage=\'url({IMAGESPATH}dialog_closebtn_over.gif)\'" style="margin: 4px 0 0;*margin-top: 5px; position: relative;top:auto; cursor: pointer; float: right; height: 17px; width: 28px; background: url({IMAGESPATH}dialog_closebtn.gif) 0 0;' + (ielt7 ? "margin-top: 3px;" : "") + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
...
Dialog.prototype.show = function () {
	...
	bgdiv=null;
    if (this.AllowZoom){
        var zoomBtn = "_ButtonZoom_" + this.ID;
        var win = $topWindow();
        win.document.getElementById(zoomBtn).style.display = "";
    }
...
Dialog.zoom = function(thisid){
    var diag = topWin.Dialog._dialogArray[topWin.Dialog._dialogArray.length - 1];
    diag.zoomStatus = !diag.zoomStatus;
    if(diag.zoomStatus){
        diag.originWidth = diag.Width;
        diag.originHeight = diag.Height;
        var bd = $bodyDimensions(topWin);
        diag.setSize(bd.clientWidth-100, bd.clientHeight-100); 
    }else
        diag.setSize(diag.originWidth, diag.originHeight);

    return window.diag = diag;
};
